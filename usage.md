...
Manage admin accounts for the lunvo_new admin panel.
...
The container is named `lunvo_new`. Run the CLI inside it with `docker exec`:

```bash
# List all admins
docker exec lunvo_new node scripts/admin-cli.js list

# Create a new admin
docker exec lunvo_new node scripts/admin-cli.js create <name> <password>

# Change an admin's password
docker exec lunvo_new node scripts/admin-cli.js update-password <name> <new-password>

# Delete an admin
docker exec lunvo_new node scripts/admin-cli.js delete <name>
```

**Examples:**

```bash
docker exec lunvo_new node scripts/admin-cli.js create alice secret123
docker exec lunvo_new node scripts/admin-cli.js update-password admin newpassword
docker exec lunvo_new node scripts/admin-cli.js delete alice
```
