...
Manage admin accounts for the smoke admin panel.
...
The container is named `smoke`. Run the CLI inside it with `docker exec`:
...
# Create a new admin
docker exec smoke node scripts/admin-cli.js create <name> <password>

# Change an admin's password
docker exec smoke node scripts/admin-cli.js update-password <name> <new-password>

# Delete an admin
docker exec smoke node scripts/admin-cli.js delete <name>
```

**Examples:**

```bash
docker exec smoke node scripts/admin-cli.js create alice secret123
docker exec smoke node scripts/admin-cli.js update-password admin newpassword
docker exec smoke node scripts/admin-cli.js delete alice
```
