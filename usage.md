...
Manage admin accounts for the steam_bot admin panel.
...
The container is named `steam_bot`. Run the CLI inside it with `docker exec`:
...
# Change an admin's password
docker exec steam_bot node scripts/admin-cli.js update-password <name> <new-password>
...
```bash
docker exec steam_bot node scripts/admin-cli.js create alice secret123
docker exec steam_bot node scripts/admin-cli.js update-password admin newpassword
docker exec steam_bot node scripts/admin-cli.js delete alice
```
