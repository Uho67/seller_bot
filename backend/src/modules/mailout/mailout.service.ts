...
export class MailoutService {
  ...
  async processQueue() {
    ...
    try {
      ...
      const { buttons, catalogCat, botSettings } = await this.senderService.getKeyboardContext();
      ...
      for (const record of batch) {
        ...
        const keyboard = this.senderService.buildKeyboard(record.post_type, buttons, catalogCat, botSettings);
